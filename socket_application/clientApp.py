...
class App:
	def __init__(self):
		...
		self.number_list = self.getRandomList(50)
	...
	def sort(self, sort:str):
		...
		if not self.sorted:
			...
			if(sort=='bubble'):
				self.list_step_by_step = self.contactServer('bubble_sort',self.number_list)
				self.animation_speed = 1
			elif(sort=='o bubble'):
				self.list_step_by_step = self.contactServer('optimised_bubble_sort',self.number_list)
				self.animation_speed = 1
			elif(sort=='selection'):
				self.list_step_by_step = self.contactServer('selection_sort',self.number_list)
				self.animation_speed = 5
			elif(sort=='insertion'):
				self.list_step_by_step = self.contactServer('insertion_sort',self.number_list)
				self.animation_speed = 5
			elif(sort=='cocktail'):
				self.list_step_by_step = self.contactServer('cocktail_sort',self.number_list)
				self.animation_speed = 1
			elif(sort=='bogo'):
				self.list_step_by_step = self.contactServer('bogo_sort',self.number_list)
				self.animation_speed = 1
			elif(sort=='count'):
				self.list_step_by_step = self.contactServer('count_sort',self.number_list)
				self.animation_speed = 5
			elif(sort=='bucket'):
				self.list_step_by_step = self.contactServer('bucket_sort',self.number_list)
				self.animation_speed = 5
		
			if(self.list_step_by_step != None):
